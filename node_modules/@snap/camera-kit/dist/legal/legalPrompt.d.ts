import type { LegalDocument, LegalDocument_Type } from "../generated-proto/pb_schema/camera_kit/v3/legal_prompt";
export type LegalPromptFactory = (privacyPolicy: LegalDocument & {
    type: LegalDocument_Type.PRIVACY_POLICY;
}, termsOfService: LegalDocument & {
    type: LegalDocument_Type.TERMS_OF_SERVICE;
}, learnMore: LegalDocument & {
    type: LegalDocument_Type.LEARN_MORE;
}, childrenProtectionActRestricted: boolean) => LegalPrompt;
export type LegalPrompt = {
    contentHash: string;
    show: () => Promise<boolean>;
};
/**
 * @internal
 */
export declare const legalPromptFactory: {
    (): LegalPromptFactory;
    token: "legalPrompt";
    dependencies: [];
};
//# sourceMappingURL=legalPrompt.d.ts.map